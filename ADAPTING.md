...
Both `opencode` and `codex` are installed unconditionally — `opencode`
via Homebrew on the host and mise in the VM; `codex` via a cask on
the host and `npm:@openai/codex` via mise in the VM.
